...
//		driver.manage().timeouts().implicitlyWait(3, TimeUnit.SECONDS);
	...
	public void clickManageRespondentsAddUser_C() throws InterruptedException {
		...
		Thread.sleep(2000);
		//----------------------------------------------
		WebElement automationTester = driver.findElement(By.xpath("//*[@class='person-name'][contains(text(),'Automation Tester')]"));
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", automationTester);
	...
	public void clickCheckBoxes() throws InterruptedException {
		...
		driver.findElement(allowComments).click();
		Thread.sleep(1000);
	}
...
//		this.verifyPollSucessResponseMessage();
	...
	By poll_Respond_Message = By.xpath("//div[@class='col-md-10 narrow meeting-details']//div//p[contains(text(),'Successfully responded to poll: This is a Poll Title')]");
	...
	public void clickSecondChoice() {
		WebElement radio = driver.findElement(By.xpath("(//input[starts-with(@id,'survey_answers_')])[2]"));

		if (!radio.isSelected()) {
			//WebElement element = driver.findElement(By.xpath("(//input[starts-with(@id,'survey_answers_')])[2]"));
	...
	public void click_Individual_Results() throws InterruptedException {
		driver.findElement(individual_Results).click();
	}
	...
	public void verify_User_Reponse_Non_Admin() throws InterruptedException {
		...
		Assert.assertTrue(driver.findElements(By.xpath(user)).size() > 0);	
		Assert.assertTrue(driver.findElements(By.xpath(check)).size() > 0);		
	}
	...
	public void verify_Poll_NOT_Exist_By_Value() throws InterruptedException {
		Thread.sleep(2000);
		String poll = "//h3//a[normalize-space(text())='This is a Poll Title']";
		Assert.assertTrue(driver.findElements(By.xpath(poll)).size() < 1);
	}
