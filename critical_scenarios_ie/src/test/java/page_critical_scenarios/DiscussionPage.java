...
public class DiscussionPage {
	...
	public void startDateCalendarPast() throws InterruptedException {
		...
		for (WebElement DATE : opt) {
			 ...
			 System.out.println(DAY);
				...
				if (DATE.getText().trim().equals(strDateOne)) {
					DATE.click(); // clicks desired option
					break;
				}
			}
	...
	By attachmentList = By.xpath("//a[contains(text(),'Attachment List')]");
	By attachFile = By.xpath("//a[@class='btn btn-primary no-mobile'][contains(text(),'Attach File')]");
	By fileTopicField = By.xpath("//input[@id='vfile_name']");
	By saveBtn2 = By.xpath("//div[@id='newDiscussionFileModal']//button[@class='btn btn-done'][contains(text(),'Save')]");
	By repliesTab = By.xpath("//a[contains(text(),'Replies')]");
	...
	public void verifyAttachmentList() {
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(20)).pollingEvery(Duration.ofMillis(600))
				.ignoring(NoSuchElementException.class);
		wait.until(ExpectedConditions.or(ExpectedConditions.elementToBeClickable(attachmentList)));
	}

	public void verifyAttachFile() {
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(20)).pollingEvery(Duration.ofMillis(600))
				.ignoring(NoSuchElementException.class);
		wait.until(ExpectedConditions.or(ExpectedConditions.elementToBeClickable(attachFile)));
	}

	public void verifyFileTopicField() {
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(20)).pollingEvery(Duration.ofMillis(600))
				.ignoring(NoSuchElementException.class);
		wait.until(ExpectedConditions.or(ExpectedConditions.elementToBeClickable(fileTopicField)));
	}

	public void verifyRepliesTab() {
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(20)).pollingEvery(Duration.ofMillis(600))
				.ignoring(NoSuchElementException.class);
		wait.until(ExpectedConditions.or(ExpectedConditions.elementToBeClickable(repliesTab)));
	}

	public void clickAttachmentList() throws InterruptedException {
		driver.findElement(attachmentList).click();
	}
	...
	public void clickRepliesTab() throws InterruptedException {
		driver.findElement(repliesTab).click();
	}

	public void File_Topic_SendKeys_And_Attach() throws InterruptedException {
		driver.findElement(fileTopicField).sendKeys("File Attached");
	...
	public void topicReplySendKeysAndAttach() throws InterruptedException {
		...
		driver.findElement(topicSubmitBtn).click();
		Thread.sleep(2000);
		this.verifyAttachmentList();
		this.clickAttachmentList();
		Thread.sleep(2000);
		this.verifyAttachFile();
		this.clickAttachFile();
		Thread.sleep(2000);
		this.verifyFileTopicField();
		this.File_Topic_SendKeys_And_Attach();
	}
	...
	public void create_Topic_Reply_And_Attachment() throws InterruptedException {
		...
		this.verifyRepliesTab();
		this.clickRepliesTab();
		Thread.sleep(2000);
		this.verifyNewReplyBtn();
	...
	public void clickArchivesBtn() throws InterruptedException {
		...
		WebElement foo = driver.findElement(By.xpath("//a[@class='btn btn-default'][contains(text(),'Archives')]"));
