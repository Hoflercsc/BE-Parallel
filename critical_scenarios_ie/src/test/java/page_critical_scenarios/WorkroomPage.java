...
public class WorkroomPage {
	...
	// (3)')]");
	By active_WorkRoom_Tab = By.xpath("//a[contains(text(),'Active Workrooms')]");
	By new_BE_Workroom = By.xpath("//a[contains(text(),'NewBEWorkRoom')]");
	String workRoomName = "NewBEWorkRoom";
	...
	public void click_New_Workroom() {
		...
		WebElement element = driver.findElement(By.xpath("//a[contains(text(),'New Workroom')]"));
		JavascriptExecutor executor = (JavascriptExecutor) driver;
		executor.executeScript("arguments[0].click();", element);
	}
	...
	public void click_Active_WorkRoom_Tab() {
		...
		WebElement element = driver.findElement(By.xpath("//a[contains(text(),'Active Workrooms')]"));
		JavascriptExecutor executor = (JavascriptExecutor) driver;
		executor.executeScript("arguments[0].click();", element);
	}
	...
	By workroom_Dropdown_Arrow = By.xpath("(//a[@class='collapsed committee-collapse placeholder'][contains(text(),'NewBEWorkRoom')]/following::div[@class='btn-group small'])[1]");
	...
	public void close_Refresh_Go_Back() throws InterruptedException {
		...
		obj.click_Close_Settings();
		Thread.sleep(1500);
		driver.navigate().refresh();
		Thread.sleep(1500);
		this.user_Opens_Settings_Opens_Workroom_And_Group_Settings();
		System.out.println("------- Completed --> close_Refresh_Go_Back() -----------");
	}
	
	// ------------------------user_Can_Delete_Workroom-----------------------------------
	By deactivated_Workrooms_Tab = By.xpath("//a[contains(text(),'Deactivated Workrooms')]");
	By deactivated_Workrooms_Dropdown_Arrow = By.xpath("//a[@class='collapsed deactive-committee-collapse placeholder'][contains(text(),'NewBEWorkRoom')]/following::button[@class='btn btn-default btn-xs dropdown-toggle']");
	...
	public void click_Deactivate_Workroom_Dropdown_Arrow() {
		...
		WebElement element = driver.findElement(By.xpath("//a[@class='collapsed deactive-committee-collapse placeholder'][contains(text(),'NewBEWorkRoom')]/following::button[@class='btn btn-default btn-xs dropdown-toggle']"));
		JavascriptExecutor executor = (JavascriptExecutor) driver;
		executor.executeScript("arguments[0].click();", element);
	}
	...
	public void click_Form_Input_Field_SendKeys() {
		driver.findElement(form_Input_Field).click();
		driver.findElement(form_Input_Field).sendKeys(workRoomName);
	}
	...
	By workroom_NOT_Present = By.xpath("//a[@class='collapsed deactive-committee-collapse placeholder'][contains(text(),'NewBEWorkRoom')]/following::button[@class='btn btn-default btn-xs dropdown-toggle']");
	...
	public void verify_Workroom_NOT_Present_Message() {
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(20)).pollingEvery(Duration.ofMillis(600))
				.ignoring(NoSuchElementException.class);
		wait.until(ExpectedConditions.or(ExpectedConditions.invisibilityOfElementLocated(workroom_NOT_Present)));
	}
